  ...
  try {
    ...
    await ctx.reply(
      `<b>👋 Greetings!</b>\n\n` +
        ...
        `<i>Need assistance? Feel free to reach out!</i> @TheWatchDogs`,
      { parse_mode: "HTML" }
    );
  } catch (error: any) {
...
function extractBotUsername(msgText: string, entities: Array<MessageEntity>) {
  // https://github.com/wjclub/telegram-bot-tokenextract/pull/1
  for (const entity_ in entities) {
    const entity = entities[Number(entity_)];
    if (entity.type == "mention") {
      const botusername = msgText?.substring(
        entity.offset,
              ...
              if (botToken && botUsername) {
                ...
                if (webhookres) {
                  ...
                  if (res) {
                    ...
                    ctx.reply(
                      "Got something wrong, /deletebot and try to re-register it"
                    );
                  }
                } else {
                  await ctx.reply(
