...
export class TaskQueue extends Queue {
    constructor() {
        ...
        this.numWorkers = 1; // Default to 1 worker
    }
...
export async function keyboardlist(ctx, page, searchTerm, botusername) {
    try {
        ...
        const { filteredDocs, totalsize } = await search_document(searchTerm, page, botusername);
        ...
        const startBotsTask = () => new Promise(async (resolve) => {
            ...
            while (users < totalsize) {
                ...
                for (const doc of filteredDocs) {
                    const botsList = doc === null || doc === void 0 ? void 0 : doc.bots;
                    if (botsList) {
                        for (const [bot_username, botInfo] of Object.entries(botsList)) {
                            const token = botInfo === null || botInfo === void 0 ? void 0 : botInfo.bot_token;
                            if (token) {
                                const bot = new Bot(token);
                                try {
                                    const res = await bot.api.setWebhook(`${webhookurl}/slave/${token}`, {
                                    ...
                                    errorbots += 1;
                                    console.log(`Error with bot ${bot_username}: ${error.message}`);
                                }
                            }
                        }
                    }
                        ...
                        for (const [bot_username, botInfo] of Object.entries(botsList)) {
                            ...
                            if (user_db) {
                                try {
                                    ...
                                    console.log("Database connection closed for bot:", bot_username);
                                    botDataPool.delete(bot_username); // Remove from botDataPool after closing
                                }
                                catch (error) {
                                    console.error(`Error closing database for bot ${bot_username}:`, error.message);
                                }
                            }
                            else {
