...
import { get_db_url_with_id_and_botusername, get_cache_fsub, insert_file, insert_user_on_slave, remove_f_sub, search_file_id, set_db_url, set_f_sub, get_cache_fsub_link, set_db_channel, get_cache_db_channel, get_cache_user_id, remove_db_channel, updateBotData, checkUserMembership, } from "../functions/dbFunctions.js";
import { cleanFileName, extractSearchTerm, keyboardlist, TaskQueue, } from "../functions/helperFunctions.js";
...
userComposer.on("callback_query:data", async (ctx) => {
    ...
    try {
        ...
        if (filetype == "try") {
            ...
            if (f_sub) {
                const isMember = await checkUserMembership(ctx, Number(f_sub), ctx.from.id);
                console.log("loging member here", isMember);
                if (isMember) {
                    const { filteredDocs } = await search_file_id(file_unique_id, ctx.me.username);
        ...
        if (calladatafile) {
            ...
            const inlineKeyboard = await keyboardlist(ctx, nextpage, searchTerm, botusername);
        ...
        //get prev page
            ...
            const inlineKeyboard = await keyboardlist(ctx, prevpage, searchTerm, botusername);
            await ctx.editMessageText(`Hey <a href="tg://user?id=${ctx.update.callback_query.message.entities[0].user.id}">${ctx.update.callback_query.message.entities[0].user.first_name}</a> , You Searched For: <code>${searchTerm}</code>`, {
                reply_markup: inlineKeyboard,
                parse_mode: "HTML",
            });
...
userComposer.chatType("private").command("start", async (ctx) => {
    try {
        ...
        if (ctx.match) {
            if (ctx.match == "start") {
                await ctx.reply(`<b>🚀 Welcome to <a href="https://t.me/${ctx.me.username}"><i>${ctx.me.first_name}</i></a>! 🚀</b>\n\n` +
                    ...
                    if (type == "doc") {
                        if (isMember) {
                            ...
                            if (inviteLink) {
                                ...
                                await ctx.reply(`<b>⚠️ It seems you’re not a member of the update channels!</b>\n\n` +
                                    `🔗 <i>To continue using the bot, please join the required channels listed below:</i>\n\n` +
                                    `Once you've joined, simply press the <b>Try Again</b> button to get the file.`, { reply_markup: inlineKeyboard, parse_mode: "HTML" });
                            }
                        }
                    }
                }
        ...
        if (ctx.msg.chat.type == "private") {
            ...
            if ((replyMessage === null || replyMessage === void 0 ? void 0 : replyMessage.document) ||
                ...
                if (replyMessage.document) {
                    await ctx.reply(`<pre language="json">id: ${replyMessage.document.file_unique_id}</pre>`, { parse_mode: "HTML" });
                }
                else if (replyMessage.video) {
                    await ctx.reply(`<pre language="json">id: ${replyMessage.video.file_unique_id}</pre>`, { parse_mode: "HTML" });
                }
                else if (replyMessage.audio) {
                    await ctx.reply(`<pre language="json">id: ${replyMessage.audio.file_unique_id}</pre>`, { parse_mode: "HTML" });
                }
            }
            else {
                await ctx.reply("Please reply to a message containing a document, video, or audio.");
            }
        }
    }
    catch (error) {
...
userComposer.chatType("private").command("set_db", async (ctx) => {
    try {
        if (ctx.match) {
            const url = await get_db_url_with_id_and_botusername(ctx.chat.id, ctx.me.username);
            if (!url) {
    ...
    try {
        if (ctx.match) {
            ...
            if (inviteLink) {
                ...
                const res = await set_f_sub(ctx.from.id, f_sub, inviteLink, ctx.me.username);
                if (res) {
...
userComposer.chatType("private").command("set_dbchannel", async (ctx) => {
    try {
        if (ctx.match) {
            ...
            if (inviteLink) {
                const res = await set_db_channel(ctx.from.id, db_channel, ctx.me.username);
                if (res) {
                    updateBotData(ctx.me.username, ctx.from.id, {
    ...
    try {
        ...
        const textSearchTask = async () => {
            ...
            if (ctx.chat.type == "private") {
                ...
                const inlineKeyboard = await keyboardlist(ctx, 1, searchparam, botusername);
                if (inlineKeyboard) {
                    await ctx.reply(`Hey <a href="tg://user?id=${(_a = ctx.from) === null || _a === void 0 ? void 0 : _a.id}">${(_b = ctx.from) === null || _b === void 0 ? void 0 : _b.first_name}</a> , You Searched For: <code>${searchparam}</code>`, {
                        reply_markup: inlineKeyboard,
                        parse_mode: "HTML",
                    });
                }
                ...
                const inlineKeyboard = await keyboardlist(ctx, 1, searchparam, botusername);
                if (inlineKeyboard) {
                    await ctx.reply(`Hey <a href="tg://user?id=${(_c = ctx.from) === null || _c === void 0 ? void 0 : _c.id}">${(_d = ctx.from) === null || _d === void 0 ? void 0 : _d.first_name}</a> , You Searched For: <code>${searchparam}</code>`, {
                        reply_markup: inlineKeyboard,
                        parse_mode: "HTML",
                    });
                }
